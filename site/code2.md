...
# 3. Move partition data to final positions using dd (optimised block size)
# 4. Record if the LVM VG was active (e.g., it contains the root filesystem)
# 5. Deactivate LVM safely (DM_DISABLE_UDEV=1) to release kernel locks
...
# 11. Reactivate LVM *ONLY* if it was active when we started.
#
...
name="bigboot"
echo "$name: script version 62 (ShellCheck Compliant)"
...
# ── Detect LVM VG & Initial State (v61 Logic) ─────────────────────────────────

_next_vg=""
_vg_was_active=false

eval "$(/usr/sbin/blkid /dev/"$next_part_name" -o udev 2>/dev/null)"
if [[ "$ID_FS_TYPE" == "LVM2_member" ]]; then
  eval "$(DM_DISABLE_UDEV=1 /usr/sbin/lvm pvs --noheadings --nameprefixes \
          -o vg_name /dev/"$next_part_name" 2>/dev/null)"
  _next_vg="$LVM2_VG_NAME"

  # Check if the kernel mapper has already activated logical volumes for this VG
  _vg_escaped="${_next_vg//-/--}"
  ...
  if [[ "$_vg_was_active" == true ]]; then
    echo "$name: LVM detected: VG $_next_vg on $next_part_name [ACTIVE IN INITRAMFS]"
  else
    echo "$name: LVM detected: VG $_next_vg on $next_part_name [INACTIVE IN INITRAMFS]"
  fi
fi
...
if [[ -n "$_next_vg" ]]; then
  echo "$name: Deactivating VG $_next_vg to release kernel locks"
  DM_DISABLE_UDEV=1 /usr/sbin/lvm vgchange --config 'global { use_lvmetad = 0 }' -an "$_next_vg" 2>&1 &
  _vg_pid=$!
  _vg_wait=0
  ...
  if kill -0 "$_vg_pid" 2>/dev/null; then
    echo "$name: WARNING: VG deactivation timed out after 10s, killing"
    kill -9 "$_vg_pid" 2>/dev/null
  else
    wait "$_vg_pid" 2>/dev/null
    echo "$name: VG deactivated successfully"
  fi
fi
...
# ══════════════════════════════════════════════════════════════════════════════
# STATEFUL LVM REACTIVATION
# ══════════════════════════════════════════════════════════════════════════════

if [[ -n "$_next_vg" ]]; then
  if [[ "$_vg_was_active" == true ]]; then
    echo "$name: Restoring VG $_next_vg to ACTIVE state for root pivot"
    /usr/sbin/lvm vgchange --config 'global { use_lvmetad = 0 }' -ay "$_next_vg" 2>&1

    echo "$name: Waiting for LVM symlinks to populate..."
    /usr/sbin/udevadm settle --timeout=15 2>/dev/null || true

    _lv_count=0
    for _dm in /dev/mapper/"${_next_vg}"-*; do
      if [[ -b "$_dm" ]]; then
    ...
    if [[ $_lv_count -gt 0 ]]; then
      echo "$name: VG $_next_vg activated ($_lv_count LVs properly mapped)."
    else
      echo "$name: WARNING: No LVs found in /dev/mapper. Root mount may fail."
    fi
  else
    echo "$name: VG $_next_vg was INACTIVE. Leaving for real OS to activate."
  fi
fi
